...
struct SegTree {
    ...
    void apply2 (int p, int t) {
        tr[p].s = (tr[p].s + (tr[p].r - tr[p].l + 1) % mod * t % mod) % mod;
        tr[p].add = (tr[p].add + t) % mod;
    } 
    ...
    void modify (int p, int l, int r, int t, int opt) {
        ...
        if (l <= mid) modify (p << 1, l, r, t, opt);
        if (mid < r) modify (p << 1 | 1, l, r, t, opt);
